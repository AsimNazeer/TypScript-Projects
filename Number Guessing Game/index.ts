...
do {
  ...
  if (life < 1) {
    console.log(chalk.red(duck));
    let cont = prompt(
      gradient.rainbow(
        "Sorry you lost the game ... Do you want to play again(y/n)"
      )
    );
  ...
  let num = prompt(
    gradient(
      "cyan",
      "red"
    )("WelCome : " + name + " -- Please Enter a Number between 0-10 : ")
  );
  ...
  if (Number.parseInt(num) === check) {
    ...
    if (Number.parseInt(num) > check) {
      console.log(
        "Ooppss You Guessed it wrong.. Your number is higher then result"
      );
      life--;
      continue;
    } else if (Number.parseInt(num) < check) {
      console.log(
        "Ooppss You Guessed it wrong.. Your number is less then result"
      );
      life--;
      continue;
    }

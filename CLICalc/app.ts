...
async function welcome() {
  const rainbowTitle = chalkAnimation.rainbow(
    "\t WelCome To My first Simple Calculator \n"
  );
...
function getOp(parm1: string): string {
  if (parm1 === " Add ") {
    return "+";
...
async function showRes() {
  ...
  spinner.success({
    text: `Your Required Answer is : ${chalk.bgBlue(result[0])}  ${getOp(
      op
    )} ${chalk.bgBlue(result[1])} = ${chalk.bgBlue(result[2])}. `,
  });
}
...
do {
  ...
  await showRes();
  repeat = prompt(
    "Do You want another Calcultation press y and press any other to exit"
  );
